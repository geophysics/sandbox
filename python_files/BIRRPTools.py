...
def convertBIRRPoutputs(bfpath,stationinfofile,station,rrstation=None,
    ...
    try:
        datfile=writedat(bfpath,egain=egain,dlgain=dlgain,dlen=dlen,
...
def readj(jfn,egain=10,dlgain=1,dlen=(50,50),magtype='bb',
...
#    for ii in range(2,10,2):
    for kk in range(4):
        for ll,nn in enumerate(range(jj+2*(kk+1)+kk*nf,jj+2*(kk+1)+(kk+1)*nf)):
            ...
            z[kk,0,ll]=float(jline[1])
            z[kk,1,ll]=float(jline[2])
            z[kk,2,ll]=float(jline[3])
    ...
    for kk in range(2):
        for ll,nn in enumerate(range(tz+2*(kk+1)+kk*nf,tz+2*(kk+1)+(kk+1)*nf)):           
            jline=jlines[nn].strip().split()
            tip[kk,0,ll]=float(jline[1])
            tip[kk,1,ll]=float(jline[2])
            tip[kk,2,ll]=float(jline[3])
...
def writedat(dirpath,egain=10,dlgain=1,dlen=[50,50],magtype='bb',
    ...
    r=['R11','R12','R21','R22']
    datfid=file(os.path.join(dirpath,ofil+'.dat'),'w')
    datfid.write(ofil+'\n')
