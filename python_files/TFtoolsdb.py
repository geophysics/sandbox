...
def stft(db,istart=0,istop=86400,nh=2**8,tstep=2**7,ng=1,df=1.0,nfbins=2**9):
...
def robustwvd(fx,nh=2**7-1,ng=2**4-1,tstep=2**4,nfbins=2**8,df=1.0,
    ...
    for tpoint,nn in enumerate(tlst):
        ...
        for fpoint,mm in enumerate(flst):
            fxmed=np.convolve(g,fxwin*np.exp(1j*2*np.pi*mlst*mm*dt),
                              mode='same')/(nh*ng)
...
def robustSmethod(fx,L=15,nh=2**7,tstep=2**5,nfbins=2**10,df=1.0,
