...
def removePeriodicNoise(filename,dt,noiseperiods,cacherate=10,save='n'):
    ...
    for kk,nperiod in enumerate(noiseperiods):
        ...
        if dfnn!=0:
            ...
            nspot=nperiodnn
        #output the peack frequency found
        filtlst.append('Found peak at : '+str(pfreq[nspot])+' Hz \n')
        
        #make nperiod the peak period
        nperiod=(1./pfreq[nspot])/dt
        
        #get the average offset of the timeseries
        dc=bx.mean()
        ...
        dlst[0]=1
        win1=bx[0:nperiod]          #first window
        nsignal=[]                     #list of windows
        nsignal.append(win1)
        for nn,ii in enumerate(nlst):
            if T-ii<nperiod:
                dlst[ii]=1
            else:
                win=nsignal[nn]+bx[ii:ii+int(nperiod)]
                nsignal.append(win)
                dlst[ii]=1

        nsuma=nsignal[-1]/(len(nsignal))
        pn=np.convolve(nsuma-dc,dlst)[0:T]
