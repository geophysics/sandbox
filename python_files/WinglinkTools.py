...
def plotResponses(outputfile,maxcol=8,plottype='all',**kwargs):
    ...
    #plot each respones in a different figure
        ...
        for ii,station in enumerate(stationlst):
            rpdict=rplst[ii]
            fig=plt.figure(ii+1,[7,8],dpi=200)
            
            
            zrxy=[np.where(rpdict['resxy'][0]!=0)[0]]
            zryx=[np.where(rpdict['resyx'][0]!=0)[0]]
            
            #plot resistivity
            axr=fig.add_subplot(gs[:4,:])
            
            axr.loglog(plst[zrxy],rpdict['resxy'][0][zrxy],ls='--',marker='s',
                       ms=4,color='b',mfc='b')
            axr.loglog(plst[zrxy],rpdict['resxy'][1][zrxy],ls=':',marker='+',
                       ms=5,color='c',mfc='c')
            axr.loglog(plst[zryx],rpdict['resyx'][0][zryx],ls='--',marker='o',
                       ms=4,color='r',mfc='r')
            axr.loglog(plst[zryx],rpdict['resyx'][1][zryx],ls=':',marker='x',
                       ms=5,color='m',mfc='m')
            ...
            axp.semilogx(plst[zrxy],rpdict['phasexy'][0][zrxy],ls='--',
                         marker='s',ms=4,color='b',mfc='b')
            axp.semilogx(plst[zrxy],rpdict['phasexy'][1][zrxy],ls=':',
                         marker='+',ms=5,color='c',mfc='c')
            axp.semilogx(plst[zryx],rpdict['phaseyx'][0][zryx],ls='--',
                         marker='o',ms=4,color='r',mfc='r')
            axp.semilogx(plst[zryx],rpdict['phaseyx'][1][zryx],ls=':',
                         marker='x',ms=5,color='m',mfc='m')
        ...
        for ss,station in enumerate(stationlst):
            for pstation in plottype:
                if station.find(pstation)>=0:
                    ...
                    pstationlst.append(ss)

        gs=gridspec.GridSpec(6,2,wspace=.05,left=.1)
        for ss in pstationlst:
            fig=plt.figure(ss,[7,7],dpi=200)
