...
def multenterbox(msg="Fill in values for the fields."
    ...
    """
    return __multfillablebox(msg,title,fields,values,mask=None,windowsize=windowsize)
...
def __multfillablebox(msg="Fill in values for the fields."
    ...
    if windowsize==None:
        ...
        screenheight=boxRoot.winfo_screenheight()
        screenwidth=boxRoot.winfo_screenwidth()
        if windowsize[0]>=screenwidth:
            windowsize[0]=screenwidth
        if windowsize[1]>=screenheight:
            windowsize[1]=screenheight

        nwindowsize=str(int(windowsize[0]))+'x'+str(int(windowsize[1]))
        boxRoot.geometry(nwindowsize)
